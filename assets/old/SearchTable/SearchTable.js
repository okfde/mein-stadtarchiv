...
export default class SearchTable extends Component {
    ...
    renderTableCellCreated(row) {
        return(
            <td>{this.formatDate(row.created)}</td>
        )
    }

    renderTableCellIBANBIC(row) {
        return(
            <td>
                {row.iban.replace(/(\S{4})/g, '$1 ').replace(/(^\s+|\s+$)/,'')}<br/>
                <small> {row.bic}</small>
            </td>
        )
    }
    ...
    formatAccountIBAN(value) {
        return value.replace(/(\S{4})/g, '$1 ').replace(/(^\s+|\s+$)/,'');
    }

    formatUnitUid(value) {
        if (value.substr(0, 2) === 'DE') {
    ...
    formatTokenIdentifier(token) {
        if (token.type === 'static') {
            return token.identifier.replace(/(\S{2})/g, "$1 ");
        }
        if (token.type === 'girocard') {
            return token.identifier.substr(0, 3) + ' ' + token.identifier.substr(3, 5) + ' ' + token.identifier.substr(8);
        }
    ...
    formatUserStatus(value) {
        if (value === 'pre-registered-start') {
            return 'RFID-Registrierung: Start';
        }
        if (value === 'pre-registered-proof') {
            return 'RFID-Registrierung: Nachweis';
        }
        if (value === 'pre-registered-details') {
            return 'RFID-Registrierung: Details';
        }
        if (value === 'registered') {
            return 'RFID-Registrierung: Registriert';
        }
        if (value === 'first-session') {
    ...
    formatAccountProof(value) {
        if (value === 'cardreader') {
            return 'Kartenleser';
        }
        if (value === 'photo') {
            return 'Photo (ungeprüft)';
        }
        if (value === 'photo-checked') {
            return 'Photo (geprüft)';
        }
        if (value === 'trusted-person') {
            return 'Vertrauenswürdige Person';
        }
        if (value === 'backend') {
            return 'Backend';
        }
